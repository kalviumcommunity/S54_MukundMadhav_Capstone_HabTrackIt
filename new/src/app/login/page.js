...
export default function Login() {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      if (authError) { setError(authError.message); } else { setSuccess('Signed in!'); window.location.href = '/dashboard'; }
  ...
  const handleGoogleLogin = async () => {
    ...
    } catch { setError('Failed to initialize Google Login.'); setLoading(false); }
  };
    ...
    <div className="flex-1 min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden bg-[#07080e]">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1.5">Log in to continue tracking your habits</p>
        </div>
        ...
        <div className="bg-[#0c0e17]/80 backdrop-blur-xl border border-white/[0.06] rounded-2xl p-6 sm:p-8 shadow-2xl">
          ...
          {/* Google */}
          <button onClick={handleGoogleLogin} disabled={loading} className="w-full bg-white hover:bg-slate-100 text-slate-900 font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-3 transition cursor-pointer disabled:opacity-50">
          ...
          <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs text-slate-500">Forgot?</span>
              </div>
              <div className="relative">
                <Lock className="absolute left-3.5 top-3.5 h-4 w-4 text-slate-500" />
                <input type={showPassword ? 'text' : 'password'} required disabled={loading} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" autoComplete="current-password" className="form-input pl-10 pr-10 text-sm" />
        ...
        <p className="text-center text-sm text-slate-400 mt-8">
          ...
          <Link href="/signup" className="text-cyan-400 hover:text-cyan-300 font-semibold hover:underline transition">Create account</Link>
        </p>
      </div>
    </div>
  );
